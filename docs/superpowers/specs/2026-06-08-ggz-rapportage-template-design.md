...
| Decision B — deliverable | .t.json / .oet / both | **Both** | `.oet` is the correct hand-authorable source; `.t.json` matches the repo convention |
...
1. **`local/GGZ Rapportage.oet`** — hand-authorable OET template definition
   (source of truth). References the two PUBLISHED archetypes; constrains
   `clinical_synopsis/Synopsis` to `1..1`; sets `reason_for_encounter` to `0..1`;
   defaults `setting` to `802`.
2. **`local/GGZ Rapportage.t.json`** — openEHR Web Template (simplified) JSON,
   `"@type": "TEMPLATE"`, to match the repo convention. Carries:
   - a freshly-minted **real UUID** in `uid`;
   - an **explicit placeholder** for the `MD5-CAM` hash in `otherDetails`
     (e.g. `"<<REGENERATE-VIA-CDR>>"`) — the canonical-form hash must be produced
     by the CDR/tooling, never hand-fabricated.

> Note: the `.t.json` web template is normally **generated** from the `.oet`/`.opt`
> by a CDR (e.g. EHRbase, Better). The hand-authored `.t.json` here is demo
> content; its `MD5-CAM` value is a placeholder until regenerated.
